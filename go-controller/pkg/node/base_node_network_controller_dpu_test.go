...
import (
	...
	"github.com/ovn-kubernetes/go-controller/pkg/config"
...
func genOVSAddPortCmd(hostIfaceName, ifaceID, mac, ip, sandboxID, podUID string) string {
	...
	return fmt.Sprintf("ovs-vsctl --timeout=30 --may-exist add-port %s %s other_config:transient=true "+
		...
		"-- --if-exists remove interface %s external_ids k8s.ovn.org/nad", config.Default.BridgeName,
...
func genOVSDelPortCmd(portName string) string {
	return fmt.Sprintf("ovs-vsctl --timeout=15 --if-exists del-port %s %s", config.Default.BridgeName, portName)
}
